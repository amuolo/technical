...
BOOST_AUTO_TEST_CASE(insert_complex_test) {
	...
	BOOST_TEST(tech::IntervalMapTest::getMap(mapping) == std::string(" 1 B  3 Z  6 C  7 A "));
}
...
BOOST_AUTO_TEST_CASE(canonicity_simple_test) {
	...
	BOOST_TEST(tech::IntervalMapTest::getMap(mapping) == std::string(" 1 B  3 A "));
};
...
BOOST_AUTO_TEST_CASE(canonicity_inject_test) {
	...
	BOOST_TEST(tech::IntervalMapTest::getMap(mapping) == std::string(" 1 B  5 A "));
};
...
BOOST_AUTO_TEST_CASE(canonicity_test) {
	...
	BOOST_TEST(tech::IntervalMapTest::getMap(mapping) == std::string(" 1 B  3 A  5 C  7 A  9 D  11 A  13 E  15 Z  23 A "));
};
...
BOOST_AUTO_TEST_CASE(short_intrusion_test) {
	...
	BOOST_TEST(tech::IntervalMapTest::getMap(mapping) == std::string(" 1 B  3 A  5 C  7 A  9 D  11 A  13 Z  14 E  17 A  19 F  21 A "));
};
...
BOOST_AUTO_TEST_CASE(long_intrusion_test) {
	...
	BOOST_TEST(tech::IntervalMapTest::getMap(mapping) == std::string(" 1 B  3 A  5 C  6 Z  20 F  21 A "));
};
...
BOOST_AUTO_TEST_CASE(type_requirements_test) {
	...
	auto mapping = tech::interval_map<myKey, myValue>(valBegin);
